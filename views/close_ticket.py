...
class CloseButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if interaction.user.id != user_id and not any(role.id in STAFF for role in interaction.user.roles): return await interaction.followup.send("Вы не можете закрыть чужой заказ", ephemeral=True)
        global msg_close
