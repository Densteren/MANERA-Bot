...
class AppointTicketButtonRender(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        async def select_callback(interaction2: discord.Interaction):
            ...
            await interaction.channel.edit(topic=f"{user_id}:{member.id}", reason=f"ПОДКЮЧЕНИЕ РЕНДЕРМЕЙКЕРА К ЗАКАЗУ ПО ЗАПРОСУ {interaction.user.name}")
...
class AppointTicketButtonAnimator(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        async def select_callback(interaction2: discord.Interaction):
            ...
            await interaction.channel.edit(topic=f"{user_id}:{member.id}", reason=f"ПОДКЮЧЕНИЕ АНИМАТОРА К ЗАКАЗУ ПО ЗАПРОСУ {interaction.user.name}")
            new_view = discord.ui.View()
            new_view.add_item(CompleteButton())
            new_view.add_item(TicketTakenButton(user=member.display_name))
            await message_aptb.edit(view=new_view)
...
class AppointTicketButtonTitleAnimator(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        async def select_callback(interaction2: discord.Interaction):
            ...
            await interaction.channel.edit(topic=f"{user_id}:{member.id}", reason=f"ПОДКЮЧЕНИЕ МУЛЬТИПЛИКАТОРА К ЗАКАЗУ ПО ЗАПРОСУ {interaction.user.name}")
            new_view = discord.ui.View()
            new_view.add_item(CompleteButton())
            new_view.add_item(TicketTakenButton(user=member.display_name))
            await message_aptb.edit(view=new_view)
